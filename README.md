...
- 视频参考生成视频。
- 图片和视频混合参考。
- Seedance 2.5 视频编辑：连接参考视频后，在 Advanced Settings 选择“视频编辑”，可按提示词替换、增加、删除或修改视频内容。
- Seedance 2.5 视频延长：连接参考视频后，在 Advanced Settings 选择“视频延长”，可向后续写参考视频。
- Ark asset ID 引用。
- 参考输入顺序调整。
- 原生音频、镜头固定、水印开关。
- 自定义随机种子（Seed），留空表示随机。
...
在视频节点的 Advanced Settings 中，可以调整模式、音频、镜头固定、水印、随机种子；选择 Seedance 2.5 时还可以选择输出格式。分辨率、画面比例、时长在节点控制面板顶部选择。

Seedance 2.5 模式说明：

| 模式 | 用途 | 参数处理 |
| --- | --- | --- |
| 参考生成 | 文生视频、参考图/视频生视频 | 使用用户选择的比例和时长 |
| 视频编辑 | 对参考视频做替换、增加、删除、修改 | 自动使用 `adaptive` 比例和智能时长（`duration=-1`） |
| 视频延长 | 对参考视频向后续写 | 自动使用 `adaptive` 比例，时长仍可选择 |
